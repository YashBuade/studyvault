...
export async function POST(request: Request) {
  try {
    ...
    logInfo("notes.restored", { userId, noteId });
    return NextResponse.json(success({ id: noteId }));
  } catch (error) {
    logError("notes.restore_failed", error);
    return NextResponse.json(failure("INTERNAL_ERROR", "Unable to restore note"), { status: 503 });
  }
}